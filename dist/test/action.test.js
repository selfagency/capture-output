...
describe('runAction', () => {
    ...
    it('writes output to file and creates directory', async () => {
        exec.exec.mockImplementation(async (_op, _args, opts) => {
            if (opts.listeners?.stdout)
                opts.listeners.stdout(Buffer.from('hello stdout\n'));
            if (opts.listeners?.stderr)
                opts.listeners.stderr(Buffer.from('warning: caution\n'));
            return 0;
        });
    ...
    it('sets failed when exit code non-zero and fail true', async () => {
        exec.exec.mockImplementation(async (_op, _args, opts) => {
            if (opts.listeners?.stderr)
                opts.listeners.stderr(Buffer.from('error: bad things\n'));
            return 2;
        });
        ...
        expect(core.setFailed.mock.calls[0][0]).toContain('error: bad things');
